...
import (
	...
	"github.com/kwesiRutledge/goControl"
...
func TestPolyhedronConstructor1(t *testing.T) {
	...
	A := mat.NewDense(3, 3, []float64{2, 0, 0, 0, 2, 0, 0, 0, 2})
	...
	//Use Constructor
	poly1 := goControl.GetPolyhedron(A, b)
	...
	for rowIndex := 0; rowIndex < M; rowIndex++ {
		for columnIndex := 0; columnIndex < N; columnIndex++ {
			if poly1.A.At(rowIndex, columnIndex) != A.At(rowIndex, columnIndex) {
				t.Errorf("poly1.A[%v,%v] = %v; want %v", rowIndex, columnIndex, poly1.A.At(rowIndex, columnIndex), A.At(rowIndex, columnIndex))
			}
		}
	}
	// if poly1.A != A {
	...
	for vectorIndex := 0; vectorIndex < poly1.Get_b().Len(); vectorIndex++ {
		if poly1.Get_b().AtVec(vectorIndex) != b.AtVec(vectorIndex) {
			t.Errorf("poly1.A[%v] = %v; want %v", vectorIndex, poly1.Get_b().AtVec(vectorIndex), b.AtVec(vectorIndex))
		}
	}
